...
let LlmListenerService = LlmListenerService_1 = class LlmListenerService {
    ...
    constructor(contextBuilder, hermesClient, eventEmitter) {
    ...
    async handleInteraction(payload) {
        ...
        try {
            const masterPrompt = await this.contextBuilder.buildContext(payload.tenantId, payload.contactId, payload.content);
            ...
            if (response.toolCalls && response.toolCalls.length > 0) {
                for (const call of response.toolCalls) {
                    const toolEvent = new tool_called_event_1.ToolCalledEvent(payload.tenantId, payload.conversationId, payload.contactId, call.name, call.arguments);
