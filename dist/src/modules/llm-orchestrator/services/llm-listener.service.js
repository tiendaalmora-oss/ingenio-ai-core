...
let LlmListenerService = LlmListenerService_1 = class LlmListenerService {
    ...
    async handleInteraction(payload) {
        ...
        if (conversation && (conversation.status === 'HANDOFF' || conversation.status === 'PAUSED' || conversation.status === 'LOST' || conversation.status === 'RESOLVED')) {
            ...
            const isCampaignTrigger = textNorm.startsWith('hola') && (textNorm.includes('quiero mas informacion') ||
        ...
        try {
            ...
            if (response.toolCalls && response.toolCalls.length > 0) {
                ...
                if (pseudoToolMatch) {
                    ...
                    await this.eventEmitter.emitAsync('tool.called', new tool_called_event_1.ToolCalledEvent(payload.tenantId, payload.conversationId, payload.contactId, `rescued_${Date.now()}`, toolName, parsedArgs));
            ...
            if (finalContent) {
                ...
                if (lower.includes('hermes no pudo') ||
